...
def settingsButton():
    ...
    label = ttk.Label(win, text="maus 48 v 6.0", style='Bold.TLabel').pack(pady=10)
    ...
    button_1= ttk.Button(WinFrame2, text="Fidelity", width=10, command=passButton, style='my.TButton' )
    button_1.pack(side=tk.LEFT, padx=8, ipady=20)

    button_settings = ttk.Button(WinFrame2, text='Efficiency', width=10, command=passButton, style='my.TButton' )
    button_settings.pack(side=tk.LEFT, padx=8, ipady=20)

    button_record = ttk.Button(WinFrame2, text="Hybrid", width=10, command=passButton, style='my.TButton' )
    button_record.pack(side=tk.RIGHT, padx=8, ipady=20)
    ...
    button_1= ttk.Button(WinFrame4, text="Single", width=10, command=passButton, style='my.TButton' )
    button_1.pack(side=tk.LEFT, padx=8, ipady=20)

    button_settings = ttk.Button(WinFrame4, text='Infinite', width=10, command=passButton, style='my.TButton' )
    button_settings.pack(side=tk.LEFT, padx=8, ipady=20)

    button_record = ttk.Button(WinFrame4, text="Custom", width=10, command=passButton, style='my.TButton' )
    button_record.pack(side=tk.RIGHT, padx=8, ipady=20)
    ...
    def changePlayHotkey(PHKResult):
        ...
        if not play_hotkey_choice == "Choose Playback Hotkey":
            if not play_hotkey_choice == recordkeytemp:
                playkeytemp = play_hotkey_choice
            else:
    ...
    def changeRecHotkey(RHKResult):
        ...
        global recordkeytemp
        if not rec_hotkey_choice == "Choose Recording Hotkey":
            if not rec_hotkey_choice == playkeytemp:
                recordkeytemp = rec_hotkey_choice
            else:
                tk.messagebox.showwarning("Hotkey In Use", "Your playback and recording hotkey cannot be the same.\n\nPlease choose another hotkey.\n")
    ...
    LoopLabel = ttk.Label(WinFrame7Labels, text="OSRS ANTIBAN MODE", style='Bold.TLabel', anchor='w').pack(side=tk.LEFT, padx=15)
    
    LoopDescription = ttk.Label(WinFrame7Labels, text="This enables more game anti-ban features.                                                                                                                                                     \nTime Randomize:  A very small (almost unnoticeable) delay is added on top of the existing recording delays.\nThis means even when looping infinitely, there will never be an identical timing pattern.").pack(side=tk.BOTTOM, fill=tk.X)

    button_1= ttk.Button(WinFrame8, text="Turn on OSRS Randomize Antiban", width=35, command=passButton, style='my.TButton' )
    button_1.pack(side=tk.LEFT, padx=8, ipady=20)

    button_record = ttk.Button(WinFrame8, text="Turn off OSRS Randomize Antiban", width=35, command=passButton, style='my.TButton' )
    button_record.pack(side=tk.LEFT, padx=8, ipady=20)


    WinFrameEND = tk.Frame(win)
    WinFrameEND.pack(pady=8)
    SettingsInfo = ttk.Label(WinFrameEND, text="Settings are immediately saved and will persist through restarts. If unchosen, Hotkeys will be defaulted.",).pack()
...
#UI: Second line underneath the Title, if desired
#currentInfo = tk.Label(Frame1, text='Sample Text')
#currentInfo.config()
#currentInfo.pack()


#UI: Images for Play and Record buttons. Disabled as it currently adds to logging file. Maybe need to change level of logs to avoid this
'''
#Images
playimage = Image.open("play.png")
resizedplayimage = playimage.resize((80,80), Image.ANTIALIAS)
finalplayimage = ImageTk.PhotoImage(resizedplayimage)

recordimage = Image.open("record.png")
resizedrecordimage = recordimage.resize((80,80), Image.ANTIALIAS)
finalrecordimage = ImageTk.PhotoImage(resizedrecordimage)
'''
...
#UI: Information
info1 = ttk.Label(Frame2, text="Record = F8    |    Playback = F12", style='my.TLabel')
info1.config()
info1.pack()
...
def on_functionf8(key):
    if (key==keyboard.Key.f12):
        print('f12 is pressed. I will call this TOGGLEPLAY') 
        ...
        print('f8 pressed which is the recording hotkey. I will call this the TOGGLEREC')
        try:
